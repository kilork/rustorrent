use crate::{
    ...
    peer::{connect_to_peer, request_message, PeerMessage, PeerState, TorrentPeerState},
    process::TorrentToken,
    request_response::RequestResponse,
    spawn_and_log_error,
    storage::TorrentStorage,
    types::{info::PieceChecksum, Message, MessageCodec, Peer},
...
pub(crate) struct PeerLoopMessage {
    ...
    pub(crate) command_loop_broker_sender: Sender<TorrentEvent>,
...
impl PeerLoopMessage {
    pub(crate) async fn bitfield(&mut self, pieces: Vec<u8>) -> Result<bool, RsbtError> {
        ...
        self.command_loop_broker_sender
            .send(TorrentEvent::PeerPieces(peer_id, pieces))
            .await?;
    ...
    pub(crate) async fn have(&mut self, piece_index: usize) -> Result<bool, RsbtError> {
        ...
        self.command_loop_broker_sender
            .send(TorrentEvent::PeerPiece(peer_id, piece_index))
            .await?;
    ...
    pub(crate) async fn unchoke(&mut self) -> Result<bool, RsbtError> {
        ...
        self.command_loop_broker_sender
            .send(TorrentEvent::PeerUnchoke(self.peer_id))
            .await?;

        debug!("[{}] send TorrentEvent::PeerUnchoke message", peer_id);
            ...
            if let Some(ref mut torrent_peer_piece) = self.torrent_piece {
                ...
                match self.piece_length.cmp(&torrent_peer_piece.len()) {
                    ...
                    Ordering::Equal => {
                        ...
                        self.command_loop_broker_sender
                            .send(TorrentEvent::PeerPieceDownloaded(
                                peer_id,
    ...
    pub(crate) async fn request(
        ...
        self.command_loop_broker_sender
            .send(TorrentEvent::PeerPieceRequest {
                peer_id,
                index,
                begin,
                length,
    ...
    pub(crate) async fn interested(&mut self) -> Result<bool, RsbtError> {
        ...
        self.command_loop_broker_sender
            .send(TorrentEvent::PeerInterested(self.peer_id))
            .await?;
